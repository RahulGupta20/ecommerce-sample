<!DOCTYPE html>
<html class="no-js" lang="en">

<head>
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <link rel="stylesheet" href="css/font-awesome.min.css">
    <link rel="stylesheet" href="css/bootstrap.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/simple-line-icons/2.4.1/css/simple-line-icons.css">
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="css/offcanvas.css">
    <title>Ecommerce-Neptrox</title>
</head>

<body>
    <!-- =============================================================
         HEADER 
     ==============================================================-->
    <header id="header" class="">
        <!-- Top-bar and tab-mobile navigation -->
        <div class="top-bar">
            <ul class="d-none d-lg-block theme-bg-dark breadcrumb rounded-0 mb-0">
                <div class="container">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="language">
                                <li class="breadcrumb-item dropdown">
                                    <a class="dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                  USD
                                </a>
                                    <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                        <a class="dropdown-item" href="#">USD</a>
                                        <a class="dropdown-item" href="#">INR</a>
                                        <a class="dropdown-item" href="#">GBP</a>
                                    </div>
                                </li>
                                <li class="breadcrumb-item dropdown">
                                    <a class="dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                  English
                                </a>
                                    <div class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
                                        <a class="dropdown-item" href="#">English</a>
                                        <a class="dropdown-item" href="#">French</a>
                                        <a class="dropdown-item" href="#">German</a>
                                    </div>
                                </li>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="login-detail float-sm-right">
                                <li class="breadcrumb-item"><a href="#">Track Orders</a></li>
                                <li class="breadcrumb-item"><a href="#">My Account</a></li>
                                <li class="breadcrumb-item"><a href="#">Wishlist</a></li>
                                <li class="breadcrumb-item"><a href="#">My Cart</a></li>
                                <li class="breadcrumb-item"><a href="#">Checkout</a></li>
                                <li class="breadcrumb-item"><a href="#">Login</a></li>
                            </div>
                        </div>
                    </div>
                </div>
            </ul>
            <nav class="navbar navbar-expand-lg navbar-dark theme-bg-dark d-lg-none">
                <div class="container">
                    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavDropdownMobile" aria-controls="navbarNavDropdownMobile" aria-expanded="false" aria-label="Toggle navigation">
                        <span class="navbar-toggler-icon"></span>
                    </button>
                    <a href="#" class="navbar-brand">SHOPNOW</a>
                    <!-- <a href="#" class="btn btn-outline-light btn-sm">
                        <i class="fa fa-key mr-1"></i> Login
                    </a>
                    <a href="#" class="btn btn-outline-light btn-sm">
                        <i class="fa fa-lock mr-1"></i> Signup
                    </a> -->
                    <a href="#" class="btn theme-bg-light text-white btn-sm mt-2 mt-sm-0">
                        <i class="fa fa-heart-o"></i>
                    </a>
                    <button class="btn theme-bg-light text-white btn-sm mt-2 mt-sm-0">
                        <i class="fa fa-shopping-cart mr-1"></i> <span class="badge badge-secondary">4</span>
                    </button>
                    <a href="#" class="btn theme-bg-light text-white btn-sm mt-2 mt-sm-0">
                        <i class="fa fa-user"></i>
                    </a>
                    <div class="collapse navbar-collapse position-relative" id="navbarNavDropdownMobile">
                        <ul class="navbar-nav">
                            <li class="nav-item active">
                                <a class="nav-link" href="#">Home <span class="sr-only">(current)</span></a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Clothing</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Electronics</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Health & Beauty</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Watches</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Jewellery</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Shoes</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Kids & Girls</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" href="#">Pages</a>
                            </li>
                            <li class="nav-item dropdown-simple-listing">
                                <a class="nav-link dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                  Simple dropdown
                                </a>
                                <div class="dropdown-menu p-0 m-0 rounded-0" aria-labelledby="navbarDropdownMenuLink">
                                    <div class="row no-gutters">
                                        <div class="col-lg-2">
                                            <div class="bg-light p-3">
                                                <h2 class="products-list-header">Men</h2>
                                                <ul class="list-unstyled products-item-list">
                                                    <li><a href="#">Dresses</a></li>
                                                    <li><a href="#">Shoes </a></li>
                                                    <li><a href="#">Jackets</a></li>
                                                    <li><a href="#">Sunglasses</a></li>
                                                    <li><a href="#">Sport Wear</a></li>
                                                    <li><a href="#">Blazers</a></li>
                                                    <li><a href="#">Shirts</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="col-lg-2">
                                            <div class="bg-white p-3">
                                                <h2 class="products-list-header">Women</h2>
                                                <ul class="list-unstyled products-item-list">
                                                    <li><a href="#">Dresses</a></li>
                                                    <li><a href="#">Shoes </a></li>
                                                    <li><a href="#">Jackets</a></li>
                                                    <li><a href="#">Sunglasses</a></li>
                                                    <li><a href="#">Sport Wear</a></li>
                                                    <li><a href="#">Blazers</a></li>
                                                    <li><a href="#">Shirts</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="col-lg-2">
                                            <div class="bg-light p-3">
                                                <h2 class="products-list-header">Men</h2>
                                                <ul class="list-unstyled products-item-list">
                                                    <li><a href="#">Dresses</a></li>
                                                    <li><a href="#">Shoes </a></li>
                                                    <li><a href="#">Jackets</a></li>
                                                    <li><a href="#">Sunglasses</a></li>
                                                    <li><a href="#">Sport Wear</a></li>
                                                    <li><a href="#">Blazers</a></li>
                                                    <li><a href="#">Shirts</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="col-lg-2">
                                            <div class="bg-white p-3">
                                                <h2 class="products-list-header">Women</h2>
                                                <ul class="list-unstyled products-item-list">
                                                    <li><a href="#">Dresses</a></li>
                                                    <li><a href="#">Shoes </a></li>
                                                    <li><a href="#">Jackets</a></li>
                                                    <li><a href="#">Sunglasses</a></li>
                                                    <li><a href="#">Sport Wear</a></li>
                                                    <li><a href="#">Blazers</a></li>
                                                    <li><a href="#">Shirts</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="col-lg-2">
                                            <div class="bg-light p-3">
                                                <h2 class="products-list-header">Men</h2>
                                                <ul class="list-unstyled products-item-list">
                                                    <li><a href="#">Dresses</a></li>
                                                    <li><a href="#">Shoes </a></li>
                                                    <li><a href="#">Jackets</a></li>
                                                    <li><a href="#">Sunglasses</a></li>
                                                    <li><a href="#">Sport Wear</a></li>
                                                    <li><a href="#">Blazers</a></li>
                                                    <li><a href="#">Shirts</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                        <div class="col-lg-2">
                                            <div class="bg-white p-3">
                                                <h2 class="products-list-header">Women</h2>
                                                <ul class="list-unstyled products-item-list">
                                                    <li><a href="#">Dresses</a></li>
                                                    <li><a href="#">Shoes </a></li>
                                                    <li><a href="#">Jackets</a></li>
                                                    <li><a href="#">Sunglasses</a></li>
                                                    <li><a href="#">Sport Wear</a></li>
                                                    <li><a href="#">Blazers</a></li>
                                                    <li><a href="#">Shirts</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                            <li class="nav-item dropdown-nav-tabs">
                                <a class="nav-link dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                testing lis
                            </a>
                                <div class="dropdown-menu p-0 m-0 rounded-0" aria-labelledby="navbarDropdownMenuLink">
                                    <div class="row no-gutters">
                                        <div class="col-md-3">
                                            <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist">
                                                <a class="nav-link active bg-light theme-color" id="v-pills-home-tab" data-toggle="pill" href="#v-pills-home" role="tab" aria-controls="v-pills-home" aria-expanded="true">
                                                    <div class="d-flex justify-content-end align-items-center">
                                                        <div class="mr-auto">Home</div>
                                                        <div class="">
                                                            <i class="fa fa-angle-right pull-right"></i>
                                                        </div>
                                                    </div>
                                                </a>
                                                <a class="nav-link" id="v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-expanded="true">
                                                    <div class="d-flex justify-content-end align-items-center">
                                                        <div class="mr-auto">Profile</div>
                                                        <div class="">
                                                            <i class="fa fa-angle-right pull-right"></i>
                                                        </div>
                                                    </div>
                                                </a>
                                                <a class="nav-link" id="v-pills-messages-tab" data-toggle="pill" href="#v-pills-messages" role="tab" aria-controls="v-pills-messages" aria-expanded="true">
                                                    <div class="d-flex justify-content-end align-items-center">
                                                        <div class="mr-auto">Message</div>
                                                        <div class="">
                                                            <i class="fa fa-angle-right pull-right"></i>
                                                        </div>
                                                    </div>
                                                </a>
                                                <a class="nav-link" id="v-pills-settings-tab" data-toggle="pill" href="#v-pills-settings" role="tab" aria-controls="v-pills-settings" aria-expanded="true">
                                                    <div class="d-flex justify-content-end align-items-center">
                                                        <div class="mr-auto">Setting</div>
                                                        <div class="">
                                                            <i class="fa fa-angle-right pull-right"></i>
                                                        </div>
                                                    </div>
                                                </a>
                                            </div>
                                        </div>
                                        <div class="col-md-9">
                                            <div class="tab-content" id="v-pills-tabContent">
                                                <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
                                                    <div class="row no-gutters">
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-white">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="tab-pane fade" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
                                                    <div class="row no-gutters">
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-white">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="tab-pane fade" id="v-pills-messages" role="tabpanel" aria-labelledby="v-pills-messages-tab">
                                                    <div class="row no-gutters">
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-white">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="tab-pane fade" id="v-pills-settings" role="tabpanel" aria-labelledby="v-pills-settings-tab">
                                                    <div class="row no-gutters">
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-white">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                        <div class="col-md-3 bg-light">
                                                            <div class="">
                                                                <ul class="list-unstyled products-item-list">
                                                                    <li><a class="nav-link" href="#">Dresses</a></li>
                                                                    <li><a class="nav-link" href="#">Shoes </a></li>
                                                                    <li><a class="nav-link" href="#">Jackets</a></li>
                                                                    <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                    <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                    <li><a class="nav-link" href="#">Blazers</a></li>
                                                                    <li><a class="nav-link" href="#">Shirts</a></li>
                                                                </ul>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </li>
                        </ul>
                    </div>
                </div>
            </nav>
        </div>
        <!-- END : Top-bar and tab-mobile navigation  -->
        <div class="py-2  py-lg-3 middle-header theme-bg-light">
            <div class="container">
                <div class="row align-items-center">
                    <div class="d-none d-lg-block col-lg-2">
                        <a class="navbar-brand p-0" href="#" style="color: #e1dcef;">SHOPNOW</a>
                    </div>
                    <div class="col-md-12 col-lg-6">
                        <div class="search-product-container">
                            <div class="input-group">
                                <input type="text" class="form-control" placeholder="Search for..." aria-label="Search for...">
                                <span class="input-group-btn">
                                    <button class="btn btn-light" type="button">
                                        <i class="fa fa-search"></i>
                                    </button>
                                </span>
                            </div>
                        </div>
                    </div>
                    <div class="d-none d-lg-block col-lg-4">
                        <div class="row align-items-center">
                            <!-- <div class="col">
                                <ul class="list-inline mb-0 text-right">
                                    <li class="list-inline-item mr-4 wishlist">
                                        <a href="#">
                                            <i class="fa fa-heart-o"></i>
                                        </a>
                                    </li>
                                    <li class="list-inline-item shopping-cart mr-3">
                                        <a href="#">
                                            <i class="fa fa-shopping-cart"></i>
                                            <span class="cart-item-count">4</span>
                                        </a>
                                    </li>
                                </ul>
                            </div> -->
                            <div class="col text-right">
                                <a href="#" class="btn theme-bg-dark text-white btn-sm mt-2 mt-sm-0">
                                    <i class="fa fa-heart-o"></i>
                                </a>
                                <button class="btn theme-bg-dark text-white btn-sm mt-2 mt-sm-0">
                                    <i class="fa fa-shopping-cart mr-1"></i> Cart <span class="badge badge-secondary">4</span>
                                </button>
                                <span class="d-none d-lg-inline-block">
                                    <a href="#" class="btn btn-outline-light btn-sm">
                                        <i class="fa fa-key mr-1"></i> Login
                                    </a>
                                    <a href="#" class="btn btn-outline-light btn-sm">
                                        <i class="fa fa-lock mr-1"></i> Signup
                                    </a>
                                </span>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <nav class="navbar navbar-expand-lg navbar-light bg-white py-sm-0 d-none d-lg-block">
            <div class="container">
                <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNavDropdown" aria-controls="navbarNavDropdown" aria-expanded="false" aria-label="Toggle navigation">
                    <span class="navbar-toggler-icon"></span>
                </button>
                <div class="collapse navbar-collapse position-relative" id="navbarNavDropdown">
                    <ul class="navbar-nav">
                        <li class="nav-item active">
                            <a class="nav-link" href="#">Home <span class="sr-only">(current)</span></a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Clothing</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Electronics</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Health & Beauty</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Watches</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Jewellery</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Shoes</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Kids & Girls</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" href="#">Pages</a>
                        </li>
                        <li class="nav-item dropdown-simple-listing">
                            <a class="nav-link dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                  Simple dropdown
                                </a>
                            <div class="dropdown-menu p-0 m-0 rounded-0" aria-labelledby="navbarDropdownMenuLink">
                                <div class="row no-gutters">
                                    <div class="col-sm-6 col-md-2">
                                        <div class="bg-light p-3">
                                            <h2 class="products-list-header">Men</h2>
                                            <ul class="list-unstyled products-item-list">
                                                <li><a href="#">Dresses</a></li>
                                                <li><a href="#">Shoes </a></li>
                                                <li><a href="#">Jackets</a></li>
                                                <li><a href="#">Sunglasses</a></li>
                                                <li><a href="#">Sport Wear</a></li>
                                                <li><a href="#">Blazers</a></li>
                                                <li><a href="#">Shirts</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-md-2">
                                        <div class="bg-white p-3">
                                            <h2 class="products-list-header">Women</h2>
                                            <ul class="list-unstyled products-item-list">
                                                <li><a href="#">Dresses</a></li>
                                                <li><a href="#">Shoes </a></li>
                                                <li><a href="#">Jackets</a></li>
                                                <li><a href="#">Sunglasses</a></li>
                                                <li><a href="#">Sport Wear</a></li>
                                                <li><a href="#">Blazers</a></li>
                                                <li><a href="#">Shirts</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-md-2">
                                        <div class="bg-light p-3">
                                            <h2 class="products-list-header">Men</h2>
                                            <ul class="list-unstyled products-item-list">
                                                <li><a href="#">Dresses</a></li>
                                                <li><a href="#">Shoes </a></li>
                                                <li><a href="#">Jackets</a></li>
                                                <li><a href="#">Sunglasses</a></li>
                                                <li><a href="#">Sport Wear</a></li>
                                                <li><a href="#">Blazers</a></li>
                                                <li><a href="#">Shirts</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-md-2">
                                        <div class="bg-white p-3">
                                            <h2 class="products-list-header">Women</h2>
                                            <ul class="list-unstyled products-item-list">
                                                <li><a href="#">Dresses</a></li>
                                                <li><a href="#">Shoes </a></li>
                                                <li><a href="#">Jackets</a></li>
                                                <li><a href="#">Sunglasses</a></li>
                                                <li><a href="#">Sport Wear</a></li>
                                                <li><a href="#">Blazers</a></li>
                                                <li><a href="#">Shirts</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-md-2">
                                        <div class="bg-light p-3">
                                            <h2 class="products-list-header">Men</h2>
                                            <ul class="list-unstyled products-item-list">
                                                <li><a href="#">Dresses</a></li>
                                                <li><a href="#">Shoes </a></li>
                                                <li><a href="#">Jackets</a></li>
                                                <li><a href="#">Sunglasses</a></li>
                                                <li><a href="#">Sport Wear</a></li>
                                                <li><a href="#">Blazers</a></li>
                                                <li><a href="#">Shirts</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                    <div class="col-sm-6 col-md-2">
                                        <div class="bg-white p-3">
                                            <h2 class="products-list-header">Women</h2>
                                            <ul class="list-unstyled products-item-list">
                                                <li><a href="#">Dresses</a></li>
                                                <li><a href="#">Shoes </a></li>
                                                <li><a href="#">Jackets</a></li>
                                                <li><a href="#">Sunglasses</a></li>
                                                <li><a href="#">Sport Wear</a></li>
                                                <li><a href="#">Blazers</a></li>
                                                <li><a href="#">Shirts</a></li>
                                            </ul>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                        <li class="nav-item dropdown-nav-tabs">
                            <a class="nav-link dropdown-toggle" href="http://example.com" id="navbarDropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                testing lis
                            </a>
                            <div class="dropdown-menu p-0 m-0 rounded-0" aria-labelledby="navbarDropdownMenuLink">
                                <div class="row no-gutters">
                                    <div class="col-md-3">
                                        <div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist">
                                            <a class="nav-link active bg-light theme-color" id="v-pills-home-tab" data-toggle="pill" href="#v-pills-home" role="tab" aria-controls="v-pills-home" aria-expanded="true">
                                                <div class="d-flex justify-content-end align-items-center">
                                                    <div class="mr-auto">Home</div>
                                                    <div class="">
                                                        <i class="fa fa-angle-right pull-right"></i>
                                                    </div>
                                                </div>
                                            </a>
                                            <a class="nav-link" id="v-pills-profile-tab" data-toggle="pill" href="#v-pills-profile" role="tab" aria-controls="v-pills-profile" aria-expanded="true">
                                                <div class="d-flex justify-content-end align-items-center">
                                                    <div class="mr-auto">Profile</div>
                                                    <div class="">
                                                        <i class="fa fa-angle-right pull-right"></i>
                                                    </div>
                                                </div>
                                            </a>
                                            <a class="nav-link" id="v-pills-messages-tab" data-toggle="pill" href="#v-pills-messages" role="tab" aria-controls="v-pills-messages" aria-expanded="true">
                                                <div class="d-flex justify-content-end align-items-center">
                                                    <div class="mr-auto">Message</div>
                                                    <div class="">
                                                        <i class="fa fa-angle-right pull-right"></i>
                                                    </div>
                                                </div>
                                            </a>
                                            <a class="nav-link" id="v-pills-settings-tab" data-toggle="pill" href="#v-pills-settings" role="tab" aria-controls="v-pills-settings" aria-expanded="true">
                                                <div class="d-flex justify-content-end align-items-center">
                                                    <div class="mr-auto">Setting</div>
                                                    <div class="">
                                                        <i class="fa fa-angle-right pull-right"></i>
                                                    </div>
                                                </div>
                                            </a>
                                        </div>
                                    </div>
                                    <div class="col-md-9">
                                        <div class="tab-content" id="v-pills-tabContent">
                                            <div class="tab-pane fade show active" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">
                                                <div class="row no-gutters">
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-white">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="tab-pane fade" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
                                                <div class="row no-gutters">
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-white">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="tab-pane fade" id="v-pills-messages" role="tabpanel" aria-labelledby="v-pills-messages-tab">
                                                <div class="row no-gutters">
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-white">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="tab-pane fade" id="v-pills-settings" role="tabpanel" aria-labelledby="v-pills-settings-tab">
                                                <div class="row no-gutters">
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-white">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3 bg-light">
                                                        <div class="">
                                                            <ul class="list-unstyled products-item-list">
                                                                <li><a class="nav-link" href="#">Dresses</a></li>
                                                                <li><a class="nav-link" href="#">Shoes </a></li>
                                                                <li><a class="nav-link" href="#">Jackets</a></li>
                                                                <li><a class="nav-link" href="#">Sunglasses</a></li>
                                                                <li><a class="nav-link" href="#">Sport Wear</a></li>
                                                                <li><a class="nav-link" href="#">Blazers</a></li>
                                                                <li><a class="nav-link" href="#">Shirts</a></li>
                                                            </ul>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </li>
                    </ul>
                </div>
            </div>
        </nav>
    </header>
    <!-- =============================================================
         END HEADER 
     ==============================================================-->